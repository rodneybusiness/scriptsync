      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {activeTab === Tab.BEATS && (
          <div className="space-y-3">
            ...
            {scene.beats.length > 0 ? (
              <>
                {/* Progress indicator for ADHD-friendly at-a-glance status */}
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-zinc-800">
                  <span className="text-xs text-zinc-500">
                    {scene.beats.filter(b => b.completed).length} of {scene.beats.length} complete
                  </span>
                  <div className="flex-1 mx-3 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-green-500 rounded-full transition-all"
                      style={{ width: `${(scene.beats.filter(b => b.completed).length / Math.max(scene.beats.length, 1)) * 100}%` }}
                    />
                  </div>
                </div>
              ...
              <div className="text-center py-6 text-zinc-600">
                ...
                <p className="text-xs italic mb-2">No beats defined for this scene</p>
                <p className="text-[10px] text-zinc-500">
                  Use AI Import or manually add beats to track story structure
                </p>
              </div>
            )}
          </div>
        ...
        {activeTab === Tab.NOTES && (
          <div className="space-y-4">
            ...
            {/* External Feedback - ADHD-friendly section divider */}
            {(relevantPageNotes.amazon.length > 0 || relevantPageNotes.pointGrey.length > 0) && (
              <div className="space-y-2 mt-6 pt-4 border-t border-zinc-800/50">
                <div className="flex items-center gap-2">
            ...
            {/* Rewrite Passes - THE BRILLIANT INTEGRATION - ADHD-friendly section divider */}
            ...
            {scene.notes.length === 0 && relevantPageNotes.amazon.length === 0 && relevantPageNotes.pointGrey.length === 0 && relevantGoals.length === 0 && (
              <div className="text-center py-6 text-zinc-600">
                ...
                <p className="text-xs italic mb-2">No notes for this scene</p>
                <p className="text-[10px] text-zinc-500">
                  Notes appear automatically from imported feedback or rewrite tracking data
                </p>
              </div>
            )}
        ...
        {activeTab === Tab.DOCTOR && (
          <div className="flex flex-col h-full">
            {/* Quick AI Actions - One-click common operations */}
            <div className="mb-3 pb-3 border-b border-zinc-800">
              <div className="flex items-center gap-2 mb-2">
                <svg className="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
                <h4 className="text-xs font-bold text-zinc-300 uppercase tracking-wide">Quick Actions</h4>
              </div>
              {/* ADHD-friendly larger touch targets */}
              <div className="grid grid-cols-2 gap-2.5">
                <button
                  onClick={handleAnalyze}
                  disabled={isAnalyzing}
                  className="flex items-center gap-2.5 px-4 py-2.5 bg-zinc-800/80 hover:bg-zinc-700 border border-zinc-700/50 rounded-lg text-sm text-zinc-300 transition-all hover:border-blue-500/50 disabled:opacity-50 disabled:cursor-not-allowed group"
                >
                  {isAnalyzing ? (
                ...
                <button
                  onClick={handleCheckContinuity}
                  disabled={isCheckingContinuity}
                  className="flex items-center gap-2.5 px-4 py-2.5 bg-zinc-800/80 hover:bg-zinc-700 border border-zinc-700/50 rounded-lg text-sm text-zinc-300 transition-all hover:border-amber-500/50 disabled:opacity-50 disabled:cursor-not-allowed group"
                >
                  {isCheckingContinuity ? (
                ...
                <button
                  onClick={handleGenerateAlt}
                  disabled={isGeneratingAlt}
                  className="flex items-center gap-2.5 px-4 py-2.5 bg-zinc-800/80 hover:bg-zinc-700 border border-zinc-700/50 rounded-lg text-sm text-zinc-300 transition-all hover:border-purple-500/50 disabled:opacity-50 disabled:cursor-not-allowed group"
                >
                  {isGeneratingAlt ? (
                  ...
                  <span>Alt Versions</span>
                </button>
                <button
                  onClick={() => {
                    setActiveTab(Tab.MORE);
                    setMoreSection('dialogue');
                  }}
                  className="flex items-center gap-2.5 px-4 py-2.5 bg-zinc-800/80 hover:bg-zinc-700 border border-zinc-700/50 rounded-lg text-sm text-zinc-300 transition-all hover:border-emerald-500/50 group"
                >
                  <svg className="w-4 h-4 text-emerald-400 group-hover:text-emerald-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                  </svg>
                  <span>Write Dialogue</span>
                </button>
              </div>

              {/* Show results inline if any action produced output - ADHD-friendly spacing */}
              {analysis && (
                <div className="mt-4 p-4 bg-blue-900/10 rounded-xl border-2 border-blue-900/40 max-h-[50vh] overflow-y-auto shadow-lg">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-blue-400 uppercase font-bold tracking-wide">Scene Analysis</span>
                    <button onClick={() => setAnalysis(null)} className="text-zinc-500 hover:text-zinc-300">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {continuityResult && (
                <div className="mt-4 p-4 bg-amber-900/10 rounded-xl border-2 border-amber-900/40 max-h-[50vh] overflow-y-auto shadow-lg">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-amber-400 uppercase font-bold tracking-wide">Continuity Check</span>
                    <button onClick={() => setContinuityResult(null)} className="text-zinc-500 hover:text-zinc-300">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                      </svg>
                    </button>
              ...
              {generatedAlt && (
                <div className="mt-4 p-4 bg-purple-900/10 rounded-xl border-2 border-purple-900/40 max-h-[50vh] overflow-y-auto shadow-lg">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-purple-400 uppercase font-bold tracking-wide">Alternative Versions</span>
                    <div className="flex items-center gap-2">
                      <button onClick={saveGeneratedAlt} className="text-[10px] text-purple-400 hover:text-purple-300 uppercase font-bold">Save</button>
                      <button onClick={() => setGeneratedAlt(null)} className="text-zinc-500 hover:text-zinc-300">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                      </button>
            ...
            {/* Chat area - no empty state, just messages */}
            <div className="flex-1 overflow-y-auto space-y-3 mb-3 min-h-[120px] p-1">
            ...
            <div className="pt-2 border-t border-zinc-800">
              ...
              {/* Suggestion Chips - AFTER send button per user request */}
              {chatHistory.length === 0 && (
                <div className="mt-4 pt-3 border-t border-zinc-800/50">
                  <div className="text-[10px] text-zinc-500 uppercase tracking-wide mb-2">Try asking about:</div>
                  <div className="flex flex-wrap gap-2">
                    {[
                      { label: 'Scene pacing', prompt: `How is the pacing in "${scene.title}"? Does it drag anywhere?` },
                      { label: 'Character voice', prompt: `Are the character voices distinct in this scene? Any dialogue that feels off?` },
                      { label: 'Subtext', prompt: `What's the subtext in this scene? What are characters really saying?` },
                      { label: 'Emotional arc', prompt: `What's the emotional journey of this scene? Does it escalate properly?` },
                      { label: 'Setup/payoff', prompt: `Are there any setups in earlier scenes that should pay off here, or vice versa?` },
                    ].map((chip) => (
                      <button
                        key={chip.label}
                        onClick={() => setChatInput(chip.prompt)}
                        className="px-3 py-1 bg-zinc-800/60 hover:bg-zinc-700 border border-zinc-700/50 hover:border-zinc-600 rounded-full text-[10px] text-zinc-400 hover:text-zinc-200 transition-all"
                      >
                        {chip.label}
                      </button>
                    ))}
              ...
              {/* AI Context - Compact badges at BOTTOM per user request */}
              <div className="flex flex-wrap items-center gap-2 mt-4 pt-3 border-t border-zinc-800/50">
                ...
                {config.tone && (
                  <span className="px-2 py-0.5 bg-purple-900/30 border border-purple-800/40 rounded text-[10px] text-purple-300">
                    {config.tone}
                  </span>
                )}
                {config.constraints && (
                  <span className="px-2 py-0.5 bg-amber-900/30 border border-amber-800/40 rounded text-[10px] text-amber-300 truncate max-w-[120px]" title={config.constraints}>
                    {config.constraints.length > 20 ? config.constraints.slice(0, 20) + '...' : config.constraints}
                  </span>
                )}
                {memoryState.corrections > 0 && (
                  <span className="px-2 py-0.5 bg-green-900/30 border border-green-800/40 rounded text-[10px] text-green-300">
                    {memoryState.corrections} learned
                  </span>
                )}
                <button
